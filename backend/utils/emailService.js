...
// Generiše HTML za email (isti za roditelja i vlasnika)
  ...
  const ostaleCeneHtml =
    selectedOstaleCene && selectedOstaleCene.length > 0
      ? selectedOstaleCene
          .map((c) => {
            const ukupnoOsoba =
              (booking.brojDece || 0) + (booking.brojRoditelja || 0);
            const ukupnaCena =
        ...
        <div class="price-item">
          <span>${c.naziv}:</span>
          <strong>
            ${c.tip === "po_osobi" ? `(${c.cena} RSD × ${ukupnoOsoba} osoba) = +${ukupnaCena} RSD` : `+${ukupnaCena} RSD`}
          </strong>
        </div>
      `;
          })
    ...
    selectedUsluge && selectedUsluge.length > 0
      ? selectedUsluge
        ...
        <div class="price-item">
          <span>${u.naziv}:</span>
          <strong>
            ${u.tip === "po_osobi" ? `(${u.cena} RSD × ${ukupnoOsoba} osoba) = +${ukupnaCena} RSD` : `+${ukupnaCena} RSD`}
          </strong>
        </div>
      `;
          })
  ...
  return `
    ...
    <head>
      ...
      <style>
        ...
        .header { background: ${isOwner ? "linear-gradient(135deg, #4caf50, #45a049)" : "linear-gradient(135deg, #ff6b4a, #ff8c6e)"}; color: white; padding: 30px; text-align: center; }
        .header h1 { margin: 0; font-size: 1.8rem; }
        .content { padding: 30px; }
        .section { margin-bottom: 25px; border-bottom: 1px solid #eee; padding-bottom: 15px; }
        .section h2 { color: ${isOwner ? "#4caf50" : "#ff6b4a"}; font-size: 1.2rem; margin-bottom: 15px; display: flex; align-items: center; gap: 8px; }
        ...
        .price-total { background: ${isOwner ? "#e8f5e9" : "#fff3e0"}; padding: 15px; border-radius: 12px; margin-top: 15px; display: flex; justify-content: space-between; font-weight: 700; font-size: 1.2rem; }
        .total-amount { color: ${isOwner ? "#4caf50" : "#ff6b4a"}; font-size: 1.4rem; }
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          <p>${isOwner ? `Poštovani/a <strong>${playroom.vlasnikId?.ime || ""} ${playroom.vlasnikId?.prezime || ""}</strong>,` : `Poštovani/a <strong>${roditelj.ime} ${roditelj.prezime}</strong>,`}</p>
          <p>${isOwner ? `Imate novu rezervaciju u vašoj igraonici <strong>${playroom.naziv}</strong>.` : `Vaša rezervacija za igraonicu <strong>${playroom.naziv}</strong> je uspešno kreirana.`}</p>
          ...
          <div class="section">
            <h2>💰 Cenovnik</h2>
            <div class="price-item">
              <span>Osnovna cena (${booking.brojDece || 1} dece × ${playroom.osnovnaCena} RSD):</span>
              <strong>${playroom.osnovnaCena * (booking.brojDece || 1)} RSD</strong>
            </div>
            ${
              ...
              playroom.cenaRoditelja.tip !== "ne_naplacuje"
                ? `
              <div class="price-item">
                <span>Roditelji (${booking.brojRoditelja} × ${playroom.cenaRoditelja.iznos} RSD):</span>
                <strong>${playroom.cenaRoditelja.iznos * booking.brojRoditelja} RSD</strong>
              </div>
            `
                : ""
            }
          ...
          <p>${isOwner ? `Možete pregledati sve rezervacije u vašem <a href="http://localhost:3000/owner-slots">vlasničkom panelu</a>.` : `Za sva pitanja, kontaktirajte igraonicu direktno na <strong>${playroom.kontaktTelefon}</strong> ili <strong>${playroom.kontaktEmail}</strong>.`}</p>
        </div>
        <div class="footer">
...
exports.sendBookingConfirmation = async (
  ...
  try {
    ...
    console.log(`✅ Email roditelju poslat na ${roditelj.email}`);
    return true;
  } catch (error) {
...
exports.sendBookingConfirmationToOwner = async (
  ...
  try {
    const html = generateEmailHtml(
      "🎉 Nova rezervacija!",
      "Neko je rezervisao termin u vašoj igraonici",
      booking,
      roditelj,
      playroom,
      timeSlot,
